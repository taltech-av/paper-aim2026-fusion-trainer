...
# -*- coding: utf-8 -*-
"""
Training script for OneFormer Fusion model.
"""
import os
import json
import glob
...
# ---------------------------------------------------------------------------
# utility functions (copied from maskformer training for reference)
# ---------------------------------------------------------------------------
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description='OneFormer Fusion Training')
    ...
    # Build model
    # optionally allow specifying the underlying detectron2 config file
    oneformer_cfg_file = config['OneFormer'].get('detectron_cfg', None)
    model = OneFormerFusion(
        ...
        config_file=oneformer_cfg_file,
    )
    model.to(device)

    # Setup optimizer
    optimizer = torch.optim.Adam(model.parameters(), 
                                lr=config['OneFormer']['clft_lr'])
